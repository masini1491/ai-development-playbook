...
| `akitaonrails/ai-memory` | Long-term agent memory / cross-agent handoff / retrieval authority | HIGH | REVIEWED | Markdown canonical memory + derived retrieval index, memory-as-untrusted-evidence boundary, typed handoff lifecycle, zero-LLM baseline, TTL/feedback/decay, audited auto-improvement pipeline | `74d2d31ebd8cca656c49f31563fac53e0b61c5cf` → `akitaonrails-ai-memory.md` |
...
| `totec448-spec/chat-on-steroids` | ChatGPT local execution runtime / persistent multi-agent orchestration / session continuity | HIGH | REVIEWED | Real ChatGPT-local execution/runtime reference with persistent workers, per-worker model/reasoning, session continuity, and pre-production Codex Desktop bridge research; strongest novel question is reusable child lifetime without sticky delegation authority | `f51acbccdd734f524799ea92bb747be765fba1e4` → `totec448-spec-chat-on-steroids.md` |
...
| `git-ai-project/git-ai` | AI code provenance / mechanical attribution | MEDIUM | REVIEWED | Git-native line/session/model attribution can complement Playbook semantic provenance; promising optional provider surface, but runtime canary remains blocked and current Codex/attribution limitations require evidence-bounded treatment | `0670e7ef27590af0e8ff5409267f3f4b09b8fcb4` → `git-ai-mechanical-provenance-2026-09-14.md` |
