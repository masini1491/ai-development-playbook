...
| Project AI mode selection | `PROJECT_MODES.md` | **Canonical two-value profile contract 已存在**：只接受 `ChatGPT-Only` 或 `ChatGPT+Codex`；未宣告代表 unresolved，不是第三種 mode。 |
...
| Research-bootstrap lifecycle / write boundary / handoff | `PROJECT_BOOTSTRAP.md` → `啟用條件（Activation Gate）`、`Research Write Allowlist`、`Exit / Handoff Gate` | Opt-in pre-implementation research lifecycle／temporary write profile；不形成第三種 Project AI mode，也不因 lifecycle 自動擴張 implementation source write authority。 |
...
| Project-native conditional activation / ordinary-task hot-path admission | `ACTIVATION_ADAPTERS.md` → `Conditional Activation / Adoption ≠ Activation` | **Canonical conditional-activation contract 已存在**：adoption不等於每個 task都 activate shared Playbook；若 current project governance提供 native gate，ordinary task可留在 project-native route。Shared reporting是另有明確宣告的窄化例外。 |
| Thin activation adapters | `ACTIVATION_ADAPTERS.md`；concrete shims：`CLAUDE.md`、`GEMINI.md`、`ANTIGRAVITY.md`、`.github/copilot-instructions.md`；machine discovery：`PLAYBOOK_INDEX.json`；deterministic drift/authority checks：`tools/playbook_check.py` + tests | **Canonical adapter contract + 4 個 concrete project-level host compatibility shims + machine-index pointers + deterministic static regression 已存在**；這些 shims 只做 bootstrap handoff，不建立 Project AI mode、execution admission／parallel authority。Antigravity executor substitution仍需 current project governance明確 mapping；native marketplace installer、startup hook、generated per-tool command pack 與 live cross-runtime behavioral conformance 仍未宣稱存在。 |
...
| Repository information architecture / durable project memory | `AI_CONTEXT.md` → `AI Context Surface Model`、`Cross-boundary Revision Continuity`、`AI Readability / Retrieval Cost Change Gate` | Always-on／Hot／Cold／Evidence／Current canonical／Historical、retrieval cost、Context Cohesion、session-local verified reuse／selective invalidation，以及跨 actor／session／execution boundary 的 minimum revision transport + bounded reconciliation；不建立獨立metadata framework。 |
| Routing integrity / discovery closure | `AI_CONTEXT.md` → `Routing Integrity Contract`；本 Playbook dogfood implementation：`tools/playbook_check.py` + tests | **Cross-project routing-integrity semantics 已存在**：structural edge、first-order discovery closure、authority/hot-path integrity、必要時 behavioral routing。Playbook checker只是本 repository 的 deterministic implementation，不是 adopter必須照抄的 universal schema。 |
| Action / synthesis-evidence / required-artifact closure | `AI_CONTEXT.md` → `Action Contract Closure`、`Material Evidence Coverage Closure` | **Canonical cross-domain closure invariants 已存在**：progressive reading可省略 irrelevant owners，但不得在 applicable minimum contract尚未 closure 前跨越action boundary；broad／multi-domain synthesis若有project-defined material evidence set，需先完成每個applicable domain的最低充分coverage disposition；required artifact則依 applicability → presence → placement → authority/evidence source → content boundary完成closure。Domain set與實際evidence semantics仍由project/domain owner擁有。 |
| Cross-actor user-facing reporting / timestamp / pre-send | `REPORTING.md` → `Applicability / Activation Boundary`、`Shared Presentation Contract`、`Reporting Timestamp`、`Shared Reporting Pre-Send Gate` | **Cross-actor／cross-workflow／activation-independent reporting owner 已存在**：substantive user-facing engineering reply共用 result-first、scope fidelity、progress integrity、absolute timestamp與 final-draft pre-send gate；clock/status/validation truth仍由其下游 owners擁有。 |
...
| Untrusted content / instruction-data authority separation | `INFORMATION_INTEGRITY.md` → `Instruction / Data Authority Separation Guard`；behavioral scenario：`evals/BEH_022_SUPPLEMENTAL.md` | Retrieved／observed content 即使含 instruction-like text，也不因 wording 自動取得 Task／Stage、tool-call、write、credential、external-service 或 data-egress authority；BEH-022 提供 targeted fresh-session regression。 |
...
| Adoption Doctor | `AGENTS.md` → `Maintainer tooling execution pointer` → `tools/README.md` → `Adoption Doctor input acquisition`；implementation：`tools/adoption_doctor.py` + `tests/test_adoption_doctor.py` | Read-only/report-only；支援 Local Path Mode 與 ChatGPT GitHub Snapshot Mode；tool-specific execution/input contract已下放到 maintainer tooling leaf。 |
| GitHub connector-first snapshot acquisition | `tools/README.md` → `ChatGPT GitHub Snapshot Mode`；authority pointer：`AGENTS.md` → `Maintainer tooling execution pointer` | Connector retrieval capability 與 local runtime/network capability分層；snapshot是 execution input，不是新 authority。 |
| GitHub operational routing / verified transport / sensitive-data history remediation / Actions / Release | `GITHUB_OPERATIONS.md` → `GitHub Operation Routing`、`Inbound Verified Transport`、`Outbound Repository Mutation`、`Sensitive Data / Secret Exposure Remediation`、`Remote Deterministic Execution`、`Tag / Release Publication` | **GitHub-specific operational owner**：把已授權的 GitHub acquisition／transport／mutation／sensitive-data recovery／remote execution／publication轉成可驗證 recipes；資料分類仍由 project governance／`INFORMATION_INTEGRITY.md` 決定，authority與 validation semantics仍由既有 owners 決定。 |
...
| Repository lower-level actor / path-action authority | `REPOSITORY_EXECUTION.md` → `Repository Actor Topology / Maintenance Ownership`；behavioral scenario：`evals/BEH_019_SUPPLEMENTAL.md` | **Normative contract + regression scenario 已存在**；selected Project AI mode決定參與的AI profile，repository governance再縮窄source/docs/tests/tooling等path/action responsibility。Lower-level authority不建立第三種Project AI mode，Playbook也不把ChatGPT→Codex handoff固定成universal workflow。 |
...
| Child delegation / subagent routing | `CODEX_EXECUTION.md` → `Delegation Opportunity Scan`、`Subagent / Delegation Gate`、`Nested Routing / Recursive Orchestration Guard`、`Parallel Multi-Agent Gate`；Prompt-side authorization：`CHATGPT_WORKFLOW.md` → `Child Delegation / Profile-Override Authorization Handoff` | **Delegation contract 已存在**；scan只找 plausible candidate，spawn仍需 bounded gate；Allowed child mutation不超過 Stage-authorized scope ∩ delegated responsibility ∩ path/action authority ∩ execution permission ∩ credential capability，nested／parallel另有獨立 gate。 |
| Child model / reasoning profile override | Prompt authorization：`CHATGPT_WORKFLOW.md` → `Child Delegation / Profile-Override Authorization Handoff`；execution semantics：`CODEX_EXECUTION.md` → `Root Profile Authority / Child Profile Override`、`Child Profile Override 回報（Completion / Final）` | **Delegation authorization ≠ profile-override authorization**；沒有 override authority時child繼承 parent profile；Execution Profile或runtime route若改變 Inference Destination，另受 `REPOSITORY_EXECUTION.md` data-egress boundary約束。Execution Profile ≠ Inference Destination；final report區分 requested/accepted override與 independently observable effective profile。 |
