...
| Action / required-artifact contract closure | `AI_CONTEXT.md` → `Action Contract Closure` | **Canonical cross-domain closure invariant 已存在**：progressive reading可省略 irrelevant owners，但不得在 applicable minimum canonical contract尚未 closure 前跨越 action boundary；若 action 產生 applicable required artifact elements，final artifact再依 applicability → presence → placement → authority/evidence source → content boundary完成最低充分 closure；unresolved只 block受影響 action。 |
