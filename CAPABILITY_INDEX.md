...
| Thin activation adapters | `ACTIVATION_ADAPTERS.md`；concrete shims：`CLAUDE.md`、`GEMINI.md`、`.github/copilot-instructions.md`；machine discovery：`PLAYBOOK_INDEX.json`；deterministic drift/authority checks：`tools/playbook_check.py` + tests | **Canonical adapter contract + 3 個 concrete project-level host compatibility shims + machine-index pointers + deterministic static regression 已存在**；這些 shims 只做 bootstrap handoff，不建立 execution admission／parallel authority。Native marketplace installer、startup hook、generated per-tool command pack 與 live cross-runtime behavioral conformance 仍未宣稱存在。 |
