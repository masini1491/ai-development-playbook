...
| Child delegation / subagent routing | `CODEX_EXECUTION.md` → `Delegation Opportunity Scan`、`Subagent / Delegation Gate`、`Nested Routing / Recursive Orchestration Guard`、`Parallel Multi-Agent Gate`；Prompt-side authorization：`CHATGPT_WORKFLOW.md` → `Prompt-authorized Child Delegation / Profile Routing` | **Delegation contract 已存在**；scan只找 plausible candidate，spawn仍需 bounded gate；child mutation authority不超過 parent Stage ∩ delegated responsibility ∩ path/action ∩ child capability，nested／parallel另有獨立 gate。 |
| Child model / reasoning profile routing | Prompt authorization：`CHATGPT_WORKFLOW.md` → `Prompt-authorized Child Delegation / Profile Routing`；execution semantics：`CODEX_EXECUTION.md` → `Root / Child Profile Routing`、`Child Profile Routing 回報（Completion / Final）` | **Delegation authorization ≠ profile-override authorization**；沒有 override authority時child繼承 parent profile；profile/router若改變 inference destination，另受 `REPOSITORY_EXECUTION.md` data-egress boundary約束。Final report區分 requested/accepted override與 independently observable effective profile。 |
