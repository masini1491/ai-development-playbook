...
| Action contract closure / pre-action sequencing | `AI_CONTEXT.md` → `Action Contract Closure` | **Canonical cross-domain sequencing invariant 已存在**：progressive reading可省略 irrelevant owners，但不得在 applicable minimum canonical contract尚未 closure 前跨越 action boundary；unresolved只 block受影響 action。 |
