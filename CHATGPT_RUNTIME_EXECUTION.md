...
- 使用者明確要求 latest/current HEAD、已有 evidence顯示 source/tool更新、dependency/runtime materially改變、asset identity無法可靠確認、probe失敗，或 correctness明確依賴 current canonical revision時，視為 **freshness trigger**：它要求退出「無條件 reuse」，**不等於 automatic reacquisition**。先 resolve current canonical revision，再依 asset 已有的 project-owned identity contract，對會 materially 影響該 asset correctness 的 source／dependency做最低充分 revision／identity check；能證明 material identity unchanged → reuse current local asset，只更新 currentness evidence；material identity changed、dependency impact unresolved、coverage不足、identity無法可信證明或 probe失敗 → 再進 exact current identity verification／canonical reacquisition。
- **Materialized provenance ≠ repository currentness observation。** 已記錄的 source commit／digest／version應描述實際 materialized bytes／artifact provenance；之後即使 current repository HEAD前進，只要 bounded identity evidence證明 material identity未變，就不得把較新的 HEAD回填成舊 local bytes 的 source provenance。需要記錄最近已確認到哪個 current revision時，使用獨立的 currentness／last-checked evidence；具體 marker欄位與schema仍由 project擁有。
